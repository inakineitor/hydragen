...
if [[ "$BENCHMARK_MODE" == "e2e" ]]; then
	...
	# ========== Flash Attention ==========
	torchrun --standalone --nproc_per_node=$NUM_MODEL_SPLITS scripts/synth.py $RESULTS_SAVE_DIR/e2e/fsynth-8b 32 1024,2048,4096,8192,16256 128 --mode hydragen_noshared --num-iters 10 --num-warmup 10 --model-name $FULL_MODEL_NAME --tp-dir "${MODEL_SAVE_DIR}/${MODEL_SPLIT_NAME}"
	torchrun --standalone --nproc_per_node=$NUM_MODEL_SPLITS scripts/synth.py $RESULTS_SAVE_DIR/e2e/fsynth-8b 64 1024,2048,4096,8192 128 --mode hydragen_noshared --num-iters 10 --num-warmup 10 --model-name $FULL_MODEL_NAME --tp-dir "${MODEL_SAVE_DIR}/${MODEL_SPLIT_NAME}"
	torchrun --standalone --nproc_per_node=$NUM_MODEL_SPLITS scripts/synth.py $RESULTS_SAVE_DIR/e2e/fsynth-8b 128 1024,2048,4096 128 --mode hydragen_noshared --num-iters 10 --num-warmup 10 --model-name $FULL_MODEL_NAME --tp-dir "${MODEL_SAVE_DIR}/${MODEL_SPLIT_NAME}"
	torchrun --standalone --nproc_per_node=$NUM_MODEL_SPLITS scripts/synth.py $RESULTS_SAVE_DIR/e2e/fsynth-8b 256 1024,2048 128 --mode hydragen_noshared --num-iters 10 --num-warmup 10 --model-name $FULL_MODEL_NAME --tp-dir "${MODEL_SAVE_DIR}/${MODEL_SPLIT_NAME}"
	torchrun --standalone --nproc_per_node=$NUM_MODEL_SPLITS scripts/synth.py $RESULTS_SAVE_DIR/e2e/fsynth-8b-c256 32 1024,2048,4096,8192,16128 256 --mode hydragen_noshared --num-iters 10 --num-warmup 10 --model-name $FULL_MODEL_NAME --tp-dir "${MODEL_SAVE_DIR}/${MODEL_SPLIT_NAME}"
	torchrun --standalone --nproc_per_node=$NUM_MODEL_SPLITS scripts/synth.py $RESULTS_SAVE_DIR/e2e/fsynth-8b-c256 64 1024,2048,4096,8192 256 --mode hydragen_noshared --num-iters 10 --num-warmup 10 --model-name $FULL_MODEL_NAME --tp-dir "${MODEL_SAVE_DIR}/${MODEL_SPLIT_NAME}"
	torchrun --standalone --nproc_per_node=$NUM_MODEL_SPLITS scripts/synth.py $RESULTS_SAVE_DIR/e2e/fsynth-8b-c256 128 1024,2048,4096 256 --mode hydragen_noshared --num-iters 10 --num-warmup 10 --model-name $FULL_MODEL_NAME --tp-dir "${MODEL_SAVE_DIR}/${MODEL_SPLIT_NAME}"
	torchrun --standalone --nproc_per_node=$NUM_MODEL_SPLITS scripts/synth.py $RESULTS_SAVE_DIR/e2e/fsynth-8b-c256 256 1024 256 --mode hydragen_noshared --num-iters 10 --num-warmup 10 --model-name $FULL_MODEL_NAME --tp-dir "${MODEL_SAVE_DIR}/${MODEL_SPLIT_NAME}"
